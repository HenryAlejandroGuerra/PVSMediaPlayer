...
{
    public sealed partial class ScribbleOverlay : OverlayForm, IOverlay
    {
        /*
        ...
        */
        ...
        public ScribbleOverlay(MainWindow baseForm, Player thePlayer)
        {
            ...
            if (_baseForm != null) AllowDrop = true;
        ...
        private void ScribbleOverlay_VisibleChanged(object sender, EventArgs e)
        {
            ...
            if (Visible)
            {
                ...
                // TODO blend on scrrencopy does not work because of shapebuffer?
        ...
        private void MoveMouse(Point location)
        {
            if (_isDrawing)
            {
                ...
                else
                {
                    ...
                    _formGraphics.DrawImageUnscaled(_shapeBuffer, 0, 0);
                }
        ...
        private void StopDrawing()
        {
            if (_isDrawing)
            {
				...
				if (_drawMode != DrawMode.Scribble)
				{
                    if (!_drawCancelled) _bufferGraphics.DrawImageUnscaled(_shapeBuffer, 0, 0);
                    else Invalidate();
				}
