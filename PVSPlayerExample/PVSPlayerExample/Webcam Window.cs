    ...
    */
    ...
    public partial class Webcam_Window : Form
    {
        ...
        public Webcam_Window(MainWindow baseForm, WebcamDevice webcam)
        {
            ...
            _webcamPlayer.DragAndDrop.Add(this);
		...
		private void Webcam_Window_Shown(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    _webcamPlayer.Events.MediaWebcamRecorderStarted += WebcamPlayer_RecorderStarted;
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                ...
                case Keys.Escape: // stop autocopy or escape full screen mode
                    if (_autoCopyTimer != null && _autoCopyTimer.Enabled)
                    {
                        StopAutoCopy();
                    }
                    else if (_webcamPlayer.FullScreen)
                    {
                        fullScreenMenuItem.PerformClick();
                    }
                    break;
        ...
        protected override void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                ...
                if (disposing)
                {
                    ...
                    if (_webcamColorDialog != null) _webcamColorDialog.Dispose();
                    if (_webcamPropertiesDialog != null) _webcamPropertiesDialog.Dispose();
                    if (_videoOverlayDialog != null) _videoOverlayDialog.Dispose();

                    Visible = false;

                    if (_audioPlayer != null) _audioPlayer.Dispose();
                    if (_webcamPlayer != null) _webcamPlayer.Dispose();
        ...
        private void WebcamPlayer_RecorderStarted(object sender, EventArgs e)
        {
            if (!screenCopyMenuItem.Checked) Icon = Properties.Resources.Media_Recording;
            recorderMenuItem.Checked = true;
            _recording = true;
        }
        ...
        private void SetWebcamWindowError(string errorText)
        {
            ...
            videoOverlayMenuItem.Enabled = false;
            ...
            if (count > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    text.Append(_formatMenuItems[i].VideoWidth)
                        ...
                        .Append(", ")
                        .Append(_formatMenuItems[i].FrameRate)
                        .Append(" fps");
        ...
        private void FormatMenu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            if (index != _selectedFormat)
            {
                ...
                if (_audioInDeviceSelected != null)
                {
                    ...
                    _audioPlayer.Play(_audioInDeviceSelected, false); // restart because of possible delay
        ...
        private void AudioInputMenu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            else
            {
                if (!((ToolStripMenuItem)audioInputMenu.Items[index]).Checked)
                {
                    ...
                    _audioPlayer.Play(_audioInDeviceSelected, false);
                    _ignoreAudioInputMenu = false;
                }
            }
            SetAudioInputMenu();
        ...
        private void AudioVolumeMenu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            _audioPlayer.Audio.VolumeTo(audioVolumeMenu.Items.IndexOf(e.ClickedItem) * 0.25f);
        }
        ...
        {
            if (_webcamColorDialog == null)
            {
                _webcamColorDialog = new VideoColorDialog(_webcamPlayer);
                _webcamColorDialog.Text += " - " + _webcam.Name;
                _webcamColorDialog.FormClosed += WebcamColorDialog_FormClosed;

                MainWindow.CenterDialog(this, _webcamColorDialog);
                _webcamColorDialog.Top = RectangleToScreen(webcamDisplay.Bounds).Bottom - _webcamColorDialog.Height - 64;

                _webcamColorDialog.Show(this);
            }
            else
            {
                _webcamColorDialog.Close();
            }
        }
        ...
        private void PropertiesMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (_webcamPropertiesDialog == null)
                {
                ...
                {
                    _webcamPropertiesDialog.Activate();
                    //_webcamPropertiesDialog.Close();
                }
            }
        }
		...
		private void CopyToClipboardMenuItem_Click(object sender, EventArgs e)
        {
            if (_scribbleOverlayOn)
            {
                ...
                _webcamPlayer.Copy.ToClipboard(CopyMode.Display);
        ...
        private void CopyToFileMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                _scribbleOverlay.MenuEnabled = false;
                _webcamPlayer.Copy.ToFile(path + string.Format("\\Image {0:yyyy-MM-dd} at {0:HH-mm-ss}.jpg", DateTime.Now), ImageFormat.Jpeg, CopyMode.Display);
                _scribbleOverlay.MenuEnabled = true;
            }
        ...
        private void StartAutoCopy(object menuItem, int interval)
        {
            ...
            if (startTimer)
            {
                if (!recorderMenuItem.Checked) Icon = Properties.Resources.Media_Recording;
                //_webcamPlayer.TaskbarProgress.State = TaskbarProgressState.Error; // just for the red color
        ...
        private void StopAutoCopy()
        {
            if (_autoCopyTimer != null)
            {
                if (!recorderMenuItem.Checked) Icon = Properties.Resources.Media_Playing;
                //_webcamPlayer.TaskbarProgress.State = TaskbarProgressState.Indeterminate;
        ...
        private void AutoCopyTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                _scribbleOverlay.MenuEnabled = false;
                _webcamPlayer.Copy.ToFile(path + string.Format("\\AutoCopy Image {0:yyyy-MM-dd} at {0:HH-mm-ss}.jpg", DateTime.Now), ImageFormat.Jpeg, CopyMode.Display);
                _scribbleOverlay.MenuEnabled = true;
            }
            else
            {
