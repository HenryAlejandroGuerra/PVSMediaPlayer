...
namespace PVSPlayerExample
{
    public partial class MainWindow
    {
        ...
        private void SetPreferences()
        {
            ...
            {
                myPlayer.Audio.Volume    = Prefs.AudioVolume;
                myPlayer.Audio.Balance   = Prefs.AudioBalance;
            }
            ...
            if (Prefs.SaveDisplayMode || (Prefs.ContinuePlay && Prefs.MediaToPlay >= 0))
            {
                ...
                {
                    if(StartPosition == FormStartPosition.Manual) myPlayer.Video.Bounds = Prefs.VideoBounds;
                    else myPlayer.Display.Mode = DisplayMode.ZoomCenter;
                }
                else myPlayer.Display.Mode = Prefs.DisplayMode;
            }

            // Speed
            if (Prefs.SaveSpeed || (Prefs.ContinuePlay && Prefs.MediaToPlay >= 0))
            {
                myPlayer.Speed.Rate = Prefs.Speed;
            }
        ...
        private void GetPreferences()
        {
            ...
            // Window
            Prefs.Fullscreen        = myPlayer.FullScreen;
            Prefs.FullScreenMode    = myPlayer.FullScreenMode;
            Prefs.Maximized         = WindowState == FormWindowState.Maximized;
            Prefs.Bounds            = myPlayer.Display.RestoreBounds;

            // Audio
            Prefs.AudioVolume       = myPlayer.Audio.Volume;
            Prefs.AudioBalance      = myPlayer.Audio.Balance;

            // DisplayMode
            Prefs.DisplayMode       = myPlayer.Display.Mode;
            Prefs.VideoBounds       = myPlayer.Video.Bounds;
            ...
            Prefs.SliderMouseWheel  = myPlayer.Sliders.Position.MouseWheel;
            ...
            Prefs.OverlayMode       = myPlayer.Overlay.Mode;

            // Speed
            Prefs.Speed             = myPlayer.Speed.Rate;

            // Continue Play
            if (Prefs.ContinuePlay && myPlayer.Playing)
            {
                // save current media (playlist entry)
                Prefs.MediaToPlay   = _mediaToPlay - 1;
                Prefs.Position      = myPlayer.Position.FromBegin.TotalMilliseconds;
                Prefs.StartPosition = myPlayer.Media.StartTime.TotalMilliseconds;
                Prefs.EndPosition   = myPlayer.Media.StopTime.TotalMilliseconds;
                Prefs.Paused        = myPlayer.Paused;
                Prefs.VideoPresent  = myPlayer.Video.Present;
            }
            else
            {
                Prefs.MediaToPlay   = -1;
